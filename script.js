...
function activarAjedrez() {
    ...
    let main = {
      ...
      methods: {
        ...
        options: function(startpoint, coordinates, piecetype) { // primero verifique si alguna de las posibles coordenadas está fuera de los límites;
          ...
          coordinates = coordinates.filter(val => {
            ...
            if (!(pos.x < 1) && !(pos.x > 8) && !(pos.y < 1) && !(pos.y > 8)) { // si no está fuera de los límites, devuelve la coordenada;
              return val;
            }
          });
          ...
          switch (piecetype) {
            ...
            case 'w_pawn':
                ...
                coordinates = coordinates.filter(val => {
                  ...
                  if (coordinate[0] < sp.x || coordinate[0] > sp.x){ // si la coordenada está a cualquier lado del centro, compruebe si tiene una pieza oponente en él;
                    return ($('#' + val).attr('chess') != 'null' && ($('#' + val).attr('chess')).slice(0,1) == 'b'); // devolver coordenadas con piezas oponentes en ellas
                    ...
                    if (coordinate[1] == (parseInt(sp.y) + 2) && $('#' + sp.x + '_' + (parseInt(sp.y) + 1)).attr('chess') != 'null'){
                      // no hacer nada si este es el primer movimiento de los peones y hay una pieza frente a la segunda coordenada;
                    } else {
                      return ($('#' + val).attr('chess') == 'null'); // de lo contrario, devuelva la coordenada si no hay una pieza de ajedrez en ella;
                    }
                  }
            ...
            case 'b_pawn':
              ...
              coordinates = coordinates.filter(val => {
                ...
                if (coordinate[0] < sp.x || coordinate[0] > sp.x){ // si la coordenada está a cualquier lado del centro, compruebe si tiene una pieza oponente en él;
                  return ($('#' + val).attr('chess') != 'null' && ($('#' + val).attr('chess')).slice(0,1) == 'w'); // devolver coordenadas con piezas oponentes en ellas
                } else { // de lo contrario, si la coordenada está en el centro;
                  if (coordinate[1] == (parseInt(sp.y) - 2) && $('#' + sp.x + '_' + (parseInt(sp.y) - 1)).attr('chess') != 'null'){
                    // no hacer nada si este es el primer movimiento de los peones y hay una pieza frente a la segunda coordenada;
                  } else {
        ...
        w_options: function (position,coordinates) {
          ...
          coordinates = coordinates.map(function(val){ // convertir x, y en coordenadas de identificación de cuadrícula reales;
              return (parseInt(position.x) + parseInt(val.x)) + '_' + (parseInt(position.y) + parseInt(val.y));
            }).filter(val => {
              let pos = { x: 0, y: 0 };
              pos.x = parseInt(val.split('_')[0]);
            ...
            }).filter(val => { // algoritmo para determinar las opciones de movimiento de línea de visión para alfil/torre/reina;
              if (flag == false){
        ...
        capture: function (target) {
          let selectedpiece = {
            ...
            // establecer la pieza seleccionada en '' de nuevo
        ...
        endturn: function(){
          ...
          if (main.variables.turn == 'w') {
            ...
            $('#turn').html("Turno de la Negras!");
    
            $('#turn').addClass('turnhighlight turnblack');
            window.setTimeout(function(){
              $('#turn').removeClass('turnhighlight turnwhite');
            }, 1500);
            ...
            $('#turn').html("Turno de la Blancas!");
    
            $('#turn').addClass('turnhighlight turnwhite');
            window.setTimeout(function(){
              $('#turn').removeClass('turnhighlight turnblack');
            }, 1500);
      ...
      $('.gamecell').click(function(e) {
        ...
        if (main.variables.selectedpiece == '' && target.name.slice(0,1) == main.variables.turn) { // show options
          ...
          if (selectedpiece.name == 'w_king' || selectedpiece.name == 'b_king'){
            ...
            } else if (t1 && t2 && t3 && t5){ // enroque b_king
          ...
          } else { // de lo contrario, si la pieza seleccionada.nombre no es rey blanco/negro, se mueve
